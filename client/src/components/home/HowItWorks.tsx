            ...
            <div className="relative grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                {homeHowItWorksData.map((step: any, i: number) => (
                    <div key={step.num} className="relative z-10 animate-slide-up" style={{ animationDelay: `${i * 100}ms` }}>
                        <div className="bg-card border border-border rounded-2xl p-8 text-center h-full shadow-sm hover:shadow-lg transition-all group/step">
                            <div className="text-5xl font-extrabold text-primary/10 mb-4 group-hover/step:text-primary/25 transition-colors">{step.num}</div>
                            <div className="size-14 rounded-xl flex items-center justify-center mx-auto mb-5 text-primary bg-accent-soft group-hover/step:scale-105 transition-transform">{step.icon}</div>
